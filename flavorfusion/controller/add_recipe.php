...
$description = isset($_POST['description']) ?
    $_POST['description'] : '';
$mealTypes = isset($_POST['mealTypes']) ?
    validate_json($_POST['mealTypes']) : [];
...
$ingredients = isset($_POST['ingredients']) ?
    validate_json($_POST['ingredients']) : [];
$instructions = isset($_POST['instructions']) ?
    validate_json($_POST['instructions']) : [];
...
if (!empty($missingFields)) {
    $message = "Missing required data for fields: " . implode
    (', ', $missingFields);
...
if (is_null($mealTypes) || is_null($ingredients) || is_null($instructions)) {
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO recipes (user_id, name,
         description, time, servings, picture) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param(
        "isssis",
        $userId,
        $title,
    ...
    if (!$stmt->execute()) {
        throw new Exception("Error inserting dietary preferences: "
            . $stmt->error);
    }
    ...
    foreach ($ingredients as $ingredient) {
        ...
        if (!empty($ingredient_name)) {
            ...
            if (!$stmt->execute()) {
                throw new Exception("Error inserting ingredient: "
                    . $stmt->error);
            }
        }
    }
    ...
    foreach ($instructions as $index => $instruction) {
        ...
        if (!empty($instruction_text)) {
            ...
            if (!$stmt->execute()) {
                throw new Exception("Error inserting instruction: "
                    . $stmt->error);
            }
        }
    }
